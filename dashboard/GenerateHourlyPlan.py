...
def meat_pieces_check(ProdMeat, req, meat):
    ...
    import json
    result = []
    meat_id_list = list(req.merge(ProdMeat, left_on='itemkey',  right_on='FinishedGoodProductCode', how='left').MeatItem.drop_duplicates())
    ...
    if len(result)==0:
        return {'status':'Feasible','result':None}
    elif len(result)>0:
        result_=pd.DataFrame(result)
        json_records = result_.reset_index().to_json(orient='records')
        arr = []
        arr = json.loads(json_records)

        return {'status': 'Infeasible', 'result': arr,'colums':list(result_.columns.values)}
...
def one_meat_checking(meat_id, result, ProdMeat, meat, req):
    ...
    print(avaliable.loc[:,'Priority 1'])
...
def activity_time_check(req,hour_priority,cap,seq):
    ...
    req_transpose=req.loc[:,["itemkey","Priority1Qty","Priority2Qty","Priority3Qty","Priority4Qty"]]
    req_transpose=req_transpose.set_index('itemkey')
    req_transpose = req_transpose.stack(0).rename_axis(('itemkey','priority')).reset_index()
    ...
    new_seq22=new_seq.loc[:,["ActivityFunctionKey","priority","TotalTimebyActivity"]]
    new_seq22.head(24)
    ...
    new_seq2.loc[new_seq2['priority'] == 'Priority1Qty', 'AvailableTime'] =  len([k for k,v in hour_priority.items() if v==1])*3600
    new_seq2.loc[new_seq2['priority'] == 'Priority2Qty', 'AvailableTime'] = len([k for k,v in hour_priority.items() if v==2])*3600
    new_seq2.loc[new_seq2['priority'] == 'Priority3Qty', 'AvailableTime'] = len([k for k,v in hour_priority.items() if v==3])*3600
    new_seq2.loc[new_seq2['priority'] == 'Priority4Qty', 'AvailableTime'] = len([k for k,v in hour_priority.items() if v==4])*3600
    ...
    new_seq3 = new_seq2.merge(cap, left_on='ActivityFunctionKey',  right_on='Activity_Num', how='left')
    new_seq3["TotalAvailableTime"]=new_seq3["AvailableTime"] * new_seq3["Max"]
    new_seq3=new_seq3.loc[:,["ActivityFunctionKey","priority","TotalTimebyActivity","TotalAvailableTime"]]
...
def plan_break_down(meat_and_req, meat_hourly, hour_priority):
    ...
    for meat_id in sorted(list(set(meat_and_req.MeatItem))):
        ...
        if len(one_meat_req)>1:
            multiple_pkg_dic, multiple_needs_by_priority = arrange_multiple_meat(one_meat_hourly, one_meat_req,t_start,t_end,hour_priority)
            #combine two lists
...
def arrange_one_meat(one_meat_hourly, one_meat_req,t_start,t_end,hour_priority):
    ...
    while t<=t_end:
        ...
        if meat_available==0: #if this hour does not produce meat and no meat remains from previous hour
            ...
            amount_can_pack = math.floor(meat_available/pieces_in_boxes) #if can produce 1.6, we round it to 1(floor)
...
def arrange_multiple_meat(one_meat_hourly, one_meat_req,t_start,t_end,hour_priority):
    ...
    for itemkey in one_meat_req.itemkey:
        ...
        names[f"pkg_dic_{itemkey}"]['itemdesc']=one_meat_req.loc[one_meat_req['itemkey']==itemkey, 'itemdesc'].values[0]
    ...
    while t<=t_end:
        ...
        if meat_available==0: #not plan
            ...
            meat_used = 0
            for itemkey in one_meat_req.itemkey:
                names[f"pkg_dic_{itemkey}"],names[f"needs_by_priority_{itemkey}"],meat_used=ratio_make_plans(t,split_p,itemkey,one_meat_req,
                                                                                                             meat_available,ratio_calculation,meat_used,
                                                                                                             names[f"pkg_dic_{itemkey}"],
                                                                                                             names[f"needs_by_priority_{itemkey}"])
            ...
            piecesinbox_required = [100000] #set a very big number to represent there's no requirement left, the statement cannot be satisfied
            for itemkey in one_meat_req.itemkey:
                boxes_still_need = names[f"needs_by_priority_{itemkey}"][1]+names[f"needs_by_priority_{itemkey}"][2]+names[f"needs_by_priority_{itemkey}"][3]+names[f"needs_by_priority_{itemkey}"][4]
            ...
            while meat_available>=min_piecesinbox_required and split_p<4 and split_priority_still_need==0:
                ...
                split_p,ratio_calculation = ratio_calculator(split_p+1,one_meat_req,needs_by_priority) #Start at next priority
...
def ratio_make_plans(t,split_p,itemkey,one_meat_req,meat_available,ratio_calculation,meat_used,
                     pkg_dic_itemkey,needs_by_priority_itemkey):
    """
    For each sku
    ...
    while t<=t_end:
        ...
        while ((len(piece_dist_byp_track)!=0) & (piece_available!=0)):
            ...
            if one_p.combo==0: #packaging only, no need to split
                ...
                new_value = piece_dist_byp_track.loc[piece_dist_byp_track['priority']==one_p.priority,'pkg']-meat_used
            ...
            if one_p.pkg==0: #combo only, no need to split
                ...
                piece_available-=meat_used
                #print(meat_used)
                new_value = piece_dist_byp_track.loc[piece_dist_byp_track['priority']==one_p.priority,'combo']-meat_used
            ...
            if ((one_p.combo!=0) & (one_p.pkg!=0)):
                ...
                new_value = piece_dist_byp_track.loc[piece_dist_byp_track['priority']==one_p.priority,'combo']-meat_used_combo
                piece_dist_byp_track.loc[piece_dist_byp_track['priority']==one_p.priority,'combo']=new_value
                ...
                new_value = piece_dist_byp_track.loc[piece_dist_byp_track['priority']==one_p.priority,'pkg']-meat_used_pkg
                piece_dist_byp_track.loc[piece_dist_byp_track['priority']==one_p.priority,'pkg']=new_value


            #update piece_dist_byp_track
            piece_dist_byp_track=piece_dist_byp_track.loc[(piece_dist_byp_track['combo']+piece_dist_byp_track['pkg'])!=0,:]
...
def plan_break_down_combo_pkg_ratio(meat_and_req, meat_hourly, hour_priority,combo_ratio):
    ...
    for meat_id in sorted(list(set(meat_and_req.MeatItem))):
        ...
        if (len(one_meat_req)>1) & (len(one_meat_req_combo)>0):#hybrid
            one_meat_hourly_combo,one_meat_hourly_pkg=meat_split_combo_pkg(hour_priority,one_meat_hourly,one_meat_req_combo,one_meat_req_pkg,combo_ratio)

            #plan for combo
            multiple_dic_combo,multiple_needs_by_priority_combo = arrange_multiple_meat(one_meat_hourly_combo,one_meat_req_combo,t_start,t_end,hour_priority)
            ...
            multiple_dic_pkg,multiple_needs_by_priority_pkg = arrange_multiple_meat(one_meat_hourly_pkg,one_meat_req_pkg,t_start,t_end,hour_priority)
...
def hourly_activity_time_check(hourly_req,cap,hour_priority,seq):
    ...
    time_required_df = req_and_activity_clean.assign(time_required=req_and_activity_clean.eval('sku_num*DurationSeconds')).groupby(['hour','Activity_Num']).sum()
    time_required_df.reset_index(inplace=True)

    #Step4: compare
    time_comparison = time_required_df.merge(cap_cleaned[["Activity_Num","hourly_available_seconds"]],how='left',left_on='Activity_Num',right_on='Activity_Num')
    time_comparison = time_comparison.assign(time_still_need = time_comparison.eval("time_required-hourly_available_seconds"))
    time_comparison=time_comparison.loc[time_comparison['time_still_need']>0,['hour','Activity_Num','time_required','hourly_available_seconds','time_still_need']]
...
def adjust_infeasible_plan(original_plan,req,cap,seq,hour_priority,adjusting_rate):
    ...
    req_cum = new_plan.loc[:,['meatkey','itemkey']].merge(req_cum.loc[:,['itemkey','p1_cum','p2_cum','p3_cum','p4_cum']],how = 'left', left_on='itemkey',right_on='itemkey')
    ...
    while current_hour<end_hour:
        ...
        if len(req_not_need_current_p)==0:#if all plans are need in current priority
            ...
            if current_hour < max(current_p_hours):
                ...
                target_req_cum = req_cum.loc[[i in target_plan['itemkey'].values for i in req_cum['itemkey']],"p"+str(current_p)+"_cum"]
                planed_more = plan_cum.values-target_req_cum
                plans_can_adjust_list = [min(target_plan[current_hour].values[i],planed_more.values[i]) for i in range(len(target_plan))]
                target_plan.loc[:,current_hour]=plans_can_adjust_list
                final_target_plan = target_plan

        else: #>0
            target_plan
            final_target_plan = target_plan.loc[[i in req_not_need_current_p['itemkey'].values for i in target_plan['itemkey']],:]
        ...
        if sum(adjustment_list)==0:
            ...
            adjustment_list = [(i == final_target_plan[current_hour].max())*1 for i in final_target_plan[current_hour]]
        final_target_plan.loc[:,current_hour] = final_target_plan.loc[:,current_hour] - adjustment_list
        final_target_plan.loc[:,current_hour+1] = final_target_plan.loc[:,current_hour+1] - adjustment_list
        #get new plan
...
# 7. Get Best plan
def get_best_plan(meat_and_req,meat_hourly,hour_priority,cap,seq,req,method1_adjusting_rate_list,method2_combo_ratio_list,method2_adjusting_rate_list):
    """
    ...
    if len(method1_adjusting_rate_list)>0:
        for adjusting_rate in method1_adjusting_rate_list:
            ...
            hourly_req, _ = plan_break_down(meat_and_req=meat_and_req, meat_hourly=meat_hourly, hour_priority=hour_priority)
            #### Activity hourly check
            infeasible_hourly_plans = hourly_activity_time_check(hourly_req,cap,hour_priority,seq)

            #### If current plan is not feasible, adjust the plan
            if len(infeasible_hourly_plans) > 0:
                new_plan,new_infeasible_table = adjust_infeasible_plan(hourly_req,req,cap,seq,hour_priority,adjusting_rate=adjusting_rate)
            ...
            dic['stdev'],plot = hourly_total_position_stdev(new_plan,cap,hour_priority,seq,plot=False)
            result.append(dic)
    ...
    if len(method2_combo_ratio_list)>0:
        for combo_ratio in method2_combo_ratio_list:
            for adjusting_rate in method2_adjusting_rate_list:
                ...
                hourly_req, _ = plan_break_down_combo_pkg_ratio(meat_and_req, meat_hourly, hour_priority,combo_ratio=combo_ratio)
                ...
                #### If current plan is not feasible, adjust the plan
                if len(infeasible_hourly_plans) > 0:
                    new_plan,new_infeasible_table = adjust_infeasible_plan(hourly_req,req,cap,seq,hour_priority,adjusting_rate=0.01)
                elif len(infeasible_hourly_plans) == 0:
...
def EEO_caculation(hourly_req,cap,hour_priority,seq):
    ...
    all_activity_time = time_required_df.merge(cap_cleaned[["Activity_Num","Activity_Desc","hourly_available_seconds","Max"]],how='left',left_on="Activity_Num",right_on="Activity_Num")
    all_activity_time = all_activity_time.assign(time_still_need = all_activity_time.eval("time_required-hourly_available_seconds"))
    all_activity_time=all_activity_time.loc[:,['hour','Activity_Num','Activity_Desc','Max','time_required','hourly_available_seconds','time_still_need']]
    ...
    result_table = activity_check_nonzero.groupby('Activity_Num').agg({'hour':['count'],'time_required':['sum'],'Max':['mean']}).reset_index()
...
def hourly_total_position_stdev(hourly_req,cap,hour_priority,seq,plot):
    ...
    #calculate time required
    time_required_df = req_and_activity_clean.assign(time_required=req_and_activity_clean.eval('sku_num*DurationSeconds')).groupby(['hour','Activity_Num']).sum().reset_index()
    ...
    all_activity_time.insert(7,"position_num",list(map(lambda x: math.ceil(x),all_activity_time['time_required']/3600)))
    ...
    #     ax.set_xticks(hour)
    #     ax.set_xlabel('hour')
    #     ax.set_ylabel('position_num')
    #     plt.gca().yaxis.set_major_locator(MaxNLocator(integer=True))
    #     ax.set_title("Hourly Total Position Num Distribution")
    #     # buffer = io.BytesIO()
...
def visualize_estimated_position_num(hourly_req,cap,hour_priority,seq):
    ...
    for activity_num in set(all_activity_time.Activity_Num):
        ...
        fig = px.bar(
            ...
            y=position_num,
            labels={'x': 'Hour', 'y': 'Position Num'},
            title=f"{activity_num}:{activity_desc}",
    ...
    #     ax.set_ylabel('position_num')
    #
    #     ax.plot([6,22], [max_cap,max_cap], 'k-', lw=2,dashes=[2, 2])
    #     plt.gca().yaxis.set_major_locator(MaxNLocator(integer=True))
    #
    #     ax.set_title(f"{activity_num}:{activity_desc}")
    #     i+=1
    #
    #     buffer = io.BytesIO()
    #     fig.savefig(buffer, format='png',transparent=True)
